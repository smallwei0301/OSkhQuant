...
## 🌐 Netlify 雲端部署版本（LazyBacktest Cloud Console）

為了服務每日約 6,000 名活躍使用者，我們新增 `web/` 目錄，提供以 React + Vite 打造的瀏覽器版本，並可透過 Netlify 一鍵部署：

1. **安裝依賴**：在 `web/` 目錄執行 `npm install`。
2. **本地測試**：使用 `npm run dev` 啟動開發伺服器，確認權益曲線、交易紀錄等功能正常。
3. **建置產出**：透過 `npm run build` 產生靜態檔案。
4. **Netlify 部署**：將倉庫連結至 Netlify，或使用 `netlify deploy`。系統會依 `netlify.toml` 自動執行 `npm run build` 並發布至 `web/dist`。

瀏覽器版本提供 CSV 上傳、RSI 與雙均線策略、手續費與資金部位設定、權益曲線圖表等完整回測流程；所有策略計算皆在使用者端完成，符合本專案「不硬編假資料、不依賴外部金流或券商 API」的原則。

> 若需進一步客製企業級方案，可依據 `web/README` 與程式內 `APP_VERSION` 註記進行版本控管與部署驗證。
