...
從版本 **LB-20240611-BE01** 起，專案新增 `backend/` 目錄，將原本依賴 PyQt 的回測框架抽離為 FastAPI + Celery 後端，可搭配 Netlify 部署的前端介面使用：

- `backend/app/main.py`：FastAPI 服務入口，提供健康檢查、策略列表與回測觸發 API。
- `backend/app/services/backtest.py`：透過 `KhQuantFramework(headless=True)` 執行回測，並轉換為 REST 回應。
- `backend/app/celery_app.py` 與 `backend/app/tasks/backtest.py`：定義 Celery 任務，支援高併發回測隊列。
- `backend/README.md`：說明佈署步驟與環境需求。

建議的部署拓樸：

1. **前端（Netlify）**：以 Next.js/React 建立響應式 UI，透過 REST 或 WebSocket 與後端互動。
2. **後端服務（雲端 VM 或 Kubernetes）**：部署本專案 `backend/` 模組，確保可連線至 xtquant 及資料源。
3. **任務隊列**：以 Redis 佈建 Celery Broker/Backend，支援批次回測與高流量使用情境。

詳細說明請參閱 [`backend/README.md`](backend/README.md)。
